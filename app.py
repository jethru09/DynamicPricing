...
def iex():
    ...
    return render_template('configure.html', industries=industries, factors=factors, influencing_factors=influencing_factors)
...
def update_columns():
    ...
    new_columns = data.get('columns')  ##  same for the inF, which is influencing_factors[selectedIndustry].
    if industry and new_columns:
        if industry in factors:
            existing_columns = set(factors[industry])
            for column in new_columns:
                if column not in existing_columns:
                    factors[industry].append(column)
                    influencing_factors[industry].append(column)
        else:
            gl_target_var = ['Sales Price']
            factors[industry] = gl_target_var + new_columns
            influencing_factors[industry] = new_columns
        return {'success': True}
    return {'success': False}, 400
